 ...
 * BUG FIX: indices and correct replacement within sliding window (incl. "synchronized" for recalcRmssd()): 2019/Aug/06
 * BUG FIX 2019/Aug/08: rrIntervalsSquaredDiffs same size as intervals array (always one index behind but same size! first calculation
 * of RMSSD can be done when there have been size+1 intervals in order to have the correct/sufficient window size of differences -> FIXED recalcRmssd());
 * tested/detailed debugging 2019/Aug/08
 */
public class HRVRmssdFromRRInstance extends AbstractRuntimeComponentInstance {
	...
	private double[] rrIntervalsSquaredDiffs = new double[propRmssdWindowSize];	// BUGFIX 2019/08/08
	...
	public Object setRuntimePropertyValue(String propertyName, Object newValue) {
		...
		if ("rmssdWindowSize".equalsIgnoreCase(propertyName)) {
			...
			rrIntervalsSquaredDiffs = new double[propRmssdWindowSize];	// BUGFIX 2019/08/08
			return oldValue;
		...
		public void receiveData(byte[] data) {
			...
			if (data.length > 0) {
				...
				if (!componentPaused) {
					...
					rrIntervalsMs[(intervalsSinceLastReset-1) % propRmssdWindowSize] = dataValue; // sliding window index - works for BUGFIX 2019/08/08 (checked/debugged!)
					recalcRmssd();					
				} else {
					AstericsErrorHandling.instance.getLogger()
	 ...
	 */
	private synchronized void recalcRmssd() {
		...
		if (componentPaused) {
		...
		} else if (intervalsSinceLastReset > propRmssdWindowSize) {	// BUGFIX 2019/08/08
			...
			// sliding window index - works for BUGFIX 2019/08/08 (checked/debugged!)
			...
			if (currentIntervalIndex > 0) {
				...
				// before is the last element of the array (which is also the index where to store the calculated squaredDiff)
				squaredDiff = rrIntervalsMs[0] - rrIntervalsMs[propRmssdWindowSize - 1];
				currentSquareSumIndex = propRmssdWindowSize - 1; // = sumRRIntervalSquaredDiffs.length()-1 = BUGFIX 2019/08/08
				AstericsErrorHandling.instance.getLogger()
			...
			if (calculationPaused) {
				...
				double squaredRmssd = sumRRIntervalSquaredDiffs / propRmssdWindowSize;	// BUGFIX 2019/08/08
